...
def get_patron_hist(phone):
	with engine.connect as con:
		query = sql.text(
			"Select B.Name as Bar_Name, dayofweek(A.timestamp) as Day, C.price*D.quantity as Amount_Spent From Bills A, Bars B, Sells C, Bought D Where A.patron_phone = :phone And A.transid = D.transid And A.bar_license = B.License And A.bar_license = C.bar_license And C.consumable_name = D.consumable_name Group by B.Name, dayofweek(A.timestamp);"
		)
